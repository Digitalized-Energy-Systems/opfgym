...
class SimpleOpfEnv(opf_env.OpfEnv):
    ...
    Constraints: Voltage band, line/trafo load, min/max reactive power, constrained reactive power flow over slack bus.
    """
    ...
    def __init__(self, simbench_network_name='1-LV-rural3--0-sw', gen_scaling=2.5, load_scaling=2.0, cos_phi=0.9, max_q_exchange=0.01, seed=None, *args, **kwargs):
        ...
        net, profiles = self._define_opf(simbench_network_name, gen_scaling=gen_scaling, load_scaling=load_scaling, *args, **kwargs)
        ...
        # n_gens = len(net['sgen'].index)

        if 'ext_grid_pen_kwargs' not in kwargs:
            kwargs['ext_grid_pen_kwargs'] = {'linear_penalty': 500}

        super().__init__(seed=seed, net=net, act_keys=act_keys, obs_keys=obs_keys, profiles=profiles, vector_reward=False, *args, **kwargs)
    ...
    def _sampling(self, step: int | None = None, test: bool = False, *args, **kwargs):
...
class QMarketEnv(opf_env.OpfEnv):
    ...
    def __init__(self, simbench_network_name: str = '1-LV-urban6--0-sw', gen_scaling: float = 2.0, load_scaling: float = 1.5, seed: int | None = None,
                 min_obs: bool = False, cos_phi: float = 0.9, max_q_exchange: float = 0.01, *args, **kwargs):
        ...
        obs_keys = [('sgen', 'p_mw', net['sgen'].index),
                    ('load', 'p_mw', net['load'].index),
                    ('load', 'q_mvar', net['load'].index),
                    ('poly_cost', 'cq2_eur_per_mvar2', np.arange(len(net.sgen) + len(net.ext_grid)))]
        ...
        super().__init__(seed=seed, net=net, obs_keys=obs_keys, act_keys=act_keys, profiles=profiles, *args, **kwargs)
    ...
    def _define_opf(self, simbench_network_name: str, *args, **kwargs) -> Tuple[pandapowerNet, Dict[tuple[str, str], DataFrame | Series]]:
        ...
        # TODO: Currently fine-tuned for simbench grid '1-LV-urban6--0-sw'
        net.ext_grid['max_q_mvar'] = self.max_q_exchange
        net.ext_grid['min_q_mvar'] = -self.max_q_exchange
    ...
    def _sampling(self, step: int | None = None, test: bool = False, *args, **kwargs) -> None:
        super()._sampling(step, test)
    ...
    def calc_objective(self, net: pandapowerNet) -> np.ndarray:
        """
...
class EcoDispatchEnv(opf_env.OpfEnv):
    """
    Economic Dispatch/Active power market environment:
    The grid operator procures active power from generators to minimize losses within its system.
    ...
    def __init__(self, simbench_network_name: str = '1-HV-urban--0-sw', min_power: int | float = 0, n_agents: Any | None = None, gen_scaling: float = 1.0,
        ...
        net, profiles = self._define_opf(simbench_network_name, min_power, n_agents, gen_scaling=gen_scaling, load_scaling=load_scaling, *args, **kwargs)
        ...
        act_keys = [('sgen', 'p_mw', net.sgen.index),  # self.sgen_idxs),
                    ('gen', 'p_mw', net.gen.index)]  # self.gen_idxs)]
        ...
        super().__init__(seed=seed, net=net, act_keys=act_keys, obs_keys=obs_keys, profiles=profiles, *args, **kwargs)
    ...
    def _sampling(self, step: int | None = None, test: bool = False, *args, **kwargs):
        super()._sampling(step, test, *args, **kwargs)
        ...
        # p_mw = net.res_ext_grid['p_mw'].to_numpy().copy()  # p_mw[p_mw < 0] = 0.0  # p_mw = np.append(  #     p_mw, net.res_sgen.p_mw.loc[self.sgen_idxs].to_numpy())  # p_mw = np.append(p_mw, net.res_gen.p_mw.loc[self.gen_idxs].to_numpy())
