...
from mlopf.opf_env import get_obs_space
...
class BiddingQMarketEnv(pettingzoo.ParallelEnv):
    ...
    def __init__(self, simbench_network_name='1-LV-urban6--0-sw'):
        ...
        self.observation_spaces = {a_id: gymnasium.spaces.Box(low=min_power[idx:idx + 1], high=max_power[idx:idx + 1]) for idx, a_id in enumerate(self.agents)}

        # Each agent has one actuator: its bidding price on the market
        self.action_spaces = {a_id: gymnasium.spaces.Box(low=-np.zeros(1), high=np.ones(1)) for idx, a_id in enumerate(self.agents)}
    ...
    def step(self, actions: dict):
        ...
        rewards = {a_id: rewards_array[idx] for idx, a_id in enumerate(self.possible_agents)}
    ...
    def _obs_to_dict(self, obss_array):
        ...
        return {a_id: np.array(obss_array[idx]).reshape(1) for idx, a_id in enumerate(self.possible_agents)}
...
class BiddingQMarketEnvBase(QMarketEnv):
    ...
    def __init__(self, simbench_network_name='1-LV-urban6--0-sw'):
        ...
        self.agent_observation_mapping = [np.array([idx]) for idx in self.net.sgen.index]
        self.agent_action_mapping = self.agent_observation_mapping

        # Overwrite action space with bid-actuators
        self.act_keys = [('poly_cost', 'cq2_eur_per_mvar2', self.net.sgen.index)]
        low = np.zeros(len(self.net.sgen.index))
        high = np.ones(len(self.net.sgen.index))
        self.action_space = gymnasium.spaces.Box(low, high)
        # Define what 100% as action means -> max price!
        self.net.poly_cost['max_max_cq2_eur_per_mvar2'] = (self.net.poly_cost.max_cq2_eur_per_mvar2)
    ...
    def _calc_reward(self, net):
        ...
        profits = np.array((reactive_prices - self.internal_costs) * net.res_sgen['q_mvar'] ** 2)
