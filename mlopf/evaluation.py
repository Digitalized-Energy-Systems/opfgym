...
def eval_experiment_folder(path: str, test_steps_int: int = 200, seed: int = 10, **kwargs) -> None:
    ...
    for idx, run_path in enumerate(run_paths):
        ...
        names.append(name)
        rewards, acts, valids, violations = get_agent_performance(agent, env, test_steps_int, seed)
    ...
    base_rewards, base_acts = get_baseline_performance(env, test_steps_int, seed)
    base_rewards = np.array(base_rewards)
    base_acts = np.array(base_acts)

    print('Get metrics \n')
    for name, validss in valids_dict.items():
        performance_comparison(name, validss, rewards_dict[name], violations_dict[name], base_rewards, True)
        performance_comparison(name, validss, rewards_dict[name], violations_dict[name], base_rewards, False)
        print('')


# TODO
def eval_one_agent(path: str, test_steps: int = 100, **kwargs) -> None:
    agent, env, name = get_agent_data(path)
    # evaluate_nstep(agent, env, test_steps)
    measure_speedup(agent, env, test_steps)


def get_agent_data(path: str) -> Tuple[Any, Any, str]:
    if path[-1] != '/':
        path += '/'

    with open(path + '/meta-data.txt', mode="r") as f:
        lines = f.readlines()
    ...
    agent = agent_class(env, name='test_agent', seed=42, path=path, **hyperparams)
    agent.load_model()
...
def performance_comparison(name, validss: list, rewardss: list, violationss: list, baseline: list, shared_mask: False):
    """
...
def measure_speedup(agent, env, test_steps, path=None):
    ...
    obss: np.ndarray = np.concatenate([env.reset(step=n).reshape(1, -1) for n in test_steps], axis=0)
    agent.test_act(obss)

    batch_time: float = round(time.time() - start_time, 3)
    batch_speedup: float = round(opf_time / batch_time, 3)
    ...
    if path:
        with open(path + 'time_measurement.txt', 'w') as f:
            ...
            f.write(f'Batch RL time: {batch_time} (speed-up: {batch_speedup})\n')
            f.write(f'Warm-start time: {rl_and_opftime} (speed-up: {rl_and_opf_speedup})\n')
    else:
        ...
        print(f'Warm-start time: {rl_and_opftime} (speed-up: {rl_and_opf_speedup})\n')
