...
def build_simbench_net(simbench_network_name: str | List[str], gen_scaling: float = 1.0, load_scaling: float = 2.0,
                       voltage_band: float = 0.05, max_loading: int = 80, *args, **kwargs) -> Tuple[pandapowerNet, Dict[tuple[str, str], DataFrame | Series]]:
    """
    Init and return a simbench power network with standard configuration.
    """

    net: pandapowerNet = sb.get_simbench_net(simbench_network_name)
    ...
    profiles: Dict[tuple[str, str], DataFrame | Series] = sb.get_absolute_values(net, profiles_instead_of_study_cases=True)
...
def set_unit_scaling(net: pandapowerNet, gen_scaling: float = 1.0, load_scaling: float = 1.0, storage_scaling: float = 1.0) -> None:
...
def set_system_constraints(net: pandapowerNet, voltage_band: float = 0.0, max_loading: float = 0.0):
...
def repair_simbench_profiles(net: pandapowerNet, profiles: Dict[tuple[str, str], DataFrame | Series]):
    """
    The simbench data sometimes contains faulty data that needs to be repaired/thrown out.
    """
    ...
    for type_act in profiles.keys():
        net_df: DataFrame = net[type_act[0]]

        is_equal: bool = profiles[type_act].max(axis=0) == profiles[type_act].min(axis=0)
        net_df.drop(net_df[is_equal].index, inplace=True)

        df: DataFrame = profiles[type_act]
        df.drop(columns=df.columns[is_equal], inplace=True)


def set_constraints_from_profiles(net: pandapowerNet, profiles: Dict[tuple[str, str], DataFrame | Series]):
    """
    Set data boundaries from profiles as constraints for the OPF and RL problem definition.
    """
    for type_act in profiles.keys():
