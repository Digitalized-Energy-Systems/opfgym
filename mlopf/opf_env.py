...
import random
import warnings
from typing import Dict, Tuple, SupportsFloat, Any
...
class OpfEnv(gymnasium.Env, ABC):

    def __init__(self,
                 net: pandapowerNet, obs_keys: list,
                 act_keys: list, profiles: Dict[tuple[str, str], DataFrame | Series],
                 train_test_split: bool = True,
                 test_share: float = 0.2,
                 vector_reward: bool = False,
                 single_step: bool = True,
                 steps_per_episode: int = 1,
                 autocorrect_prio: str = 'p_mw',
                 pf_for_obs: bool = False,
                 ...
                 volt_pen_kwargs: dict | None = None,
                 line_pen_kwargs: dict | None = None,
                 trafo_pen_kwargs: dict | None = None,
                 ext_grid_pen_kwargs: dict | None = None,
                 seed: int | None = None,
                 squash_reward: bool = False):

        # Should be always True. Maybe only allow False for paper investigation
        if line_pen_kwargs is None:
            line_pen_kwargs = {}

        self.info: dict = {}
        self.train_test_split: bool = train_test_split
        self.train_data: str = train_data
        self.test_data: str = test_data

        # upperclass defined attributs
        self.net: pandapowerNet = net
        self.obs_keys: list = obs_keys
        self.act_keys: list = act_keys
        self.profiles: Dict[tuple[str, str], DataFrame | Series] = profiles

        self.sampling_kwargs: dict = {}

        self.step_in_episode: int = 0
        self.prev_reward: np.ndarray | None = None

        if sampling_kwargs:
            self.sampling_kwargs.update(sampling_kwargs)
        ...
        if add_res_obs:
            self.obs_keys.extend([('res_bus', 'vm_pu', self.net.bus.index),
        ...
        self.bus_wise_obs: bool = bus_wise_obs
        self.observation_space: Space[ObsType] = get_obs_space(self.net, self.obs_keys, add_time_obs, seed, bus_wise_obs=bus_wise_obs)
        self.action_space: Space[ActType] = get_action_space(self.act_keys, seed)

        self.reward_function: str = reward_function
        self.vector_reward: bool = vector_reward
        self.squash_reward: bool = squash_reward

        if reward_scaling == 'auto':
            reward_scaling: float = get_automatic_reward_scaling(self.net)

        self.reward_scaling: float = reward_scaling

        # Default penalties are purely linear
        self.volt_pen: dict = volt_pen_kwargs if volt_pen_kwargs is not None else {'linear_penalty': 300}
        self.line_pen: dict = line_pen_kwargs if line_pen_kwargs is not None else {'linear_penalty': 2}
        self.trafo_pen: dict = trafo_pen_kwargs if trafo_pen_kwargs is not None else {'linear_penalty': 2}
        self.ext_grid_pen: dict = ext_grid_pen_kwargs if ext_grid_pen_kwargs is not None else {'linear_penalty': 100}

        self.priority: str = autocorrect_prio
        ...
        self.pf_for_obs: bool = pf_for_obs
        ...
        self.diff_reward: bool = diff_reward

        if self.diff_reward:
            self.pf_for_obs = True

        self.test_steps: np.ndarray = define_test_steps(test_share)
    ...
    def reset(self, step=None, test=False):
        ...
        if self.pf_for_obs is True:
            ...
            self.prev_reward = self.calc_reward(test=test)

        return self._get_obs(self.obs_keys, self.add_time_obs), self.info

    def _sampling(self, step: int | None, test: bool, *args, **kwargs) -> bool:
        """
        ...
        if data_distr == 'noisy_simbench' or 'noise_factor' in kwargs.keys():
            return self._set_simbench_state(step, test, *args, **kwargs)

        elif data_distr == 'simbench':
            return self._set_simbench_state(step, test, noise_factor=0.0, *args, **kwargs)
    ...
    def _sample_uniform(self, sample_keys=None) -> bool:
        """
        Standard pre-implemented method to set power system to a new random state from uniform sampling.
        Uses the observation space as basis.
    ...
    def _sample_from_range(self, unit_type: str, column: str, idxs: int | list) -> None:
        df: DataFrame = self.net[unit_type]
        # Make sure to sample from the biggest possible range.
        try:
            low: float = df[f'min_min_{column}'].loc[idxs]

        except KeyError:
            low: float = df[f'min_{column}'].loc[idxs]

        try:
            high: float = df[f'max_max_{column}'].loc[idxs]

        except KeyError:
            high: float = df[f'max_{column}'].loc[idxs]

        r: np.ndarray = np.random.uniform(low, high, size=(len(idxs),))
    ...
    def _sample_normal(self, std: float = 0.3, truncated: bool = False) -> bool:
        """
        ...
        for unit_type, column, idxs in self.obs_keys:
            ...
            if 'res_' not in unit_type and 'poly_cost' not in unit_type:
                df: DataFrame = self.net[unit_type].loc[idxs]
                mean = df[f'mean_{column}']
                max_values: np.ndarray = (df[f'max_max_{column}'] / df.scaling).to_numpy()
                min_values: np.ndarray = (df[f'min_min_{column}'] / df.scaling).to_numpy()
                diff = max_values - min_values

                if truncated:
                    random_values: np.ndarray = stats.truncnorm.rvs(min_values, max_values, mean, std * diff, len(mean))

                else:
                    random_values: np.ndarray = np.random.normal(mean, std * diff, len(mean))
                    random_values: np.ndarray = np.clip(random_values, min_values, max_values)

                self.net[unit_type][column].loc[idxs] = random_values
        return True

    def _set_simbench_state(self, step: int | None = None, test: bool = False, noise_factor=0.1, noise_distribution='uniform', *args, **kwargs) -> bool:
        """
    ...
    def step(self, action: np.ndarray, test: bool = False) -> Tuple[ObsType, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        if self.single_step:
            ...
            done = self.step_in_episode >= self.steps_per_episode
    ...
    def _apply_actions(self, action: np.ndarray, autocorrect: bool = False) -> None:
        """
        Apply agent actions to the power system at hand.
        """
        counter = 0
        # Clip invalid actions
        action: np.ndarray = np.clip(action, self.action_space.low[:len(action)], self.action_space.high[:len(action)])

        for unit_type, actuator, idxs in self.act_keys:
            df: DataFrame = self.net[unit_type]
            a: np.ndarray = action[counter:counter + len(idxs)]
            ...
            max_action: Series = df[f'max_max_{actuator}'].loc[idxs]
            new_values: Series = a * max_action

            # Autocorrect impossible setpoints (however: no penalties this way)
            if f'max_{actuator}' in df.columns:
                mask: Series = new_values > df[f'max_{actuator}'].loc[idxs]
                new_values[mask] = df[f'max_{actuator}'].loc[idxs][mask]

            if f'min_{actuator}' in df.columns:
                mask: Series = new_values < df[f'min_{actuator}'].loc[idxs]
    ...
    def _autocorrect_apparent_power(self, priority='p_mw') -> None:
        """
        Autocorrect to maximum apparent power if necessary.
        Relevant for sgens, loads, and storages.
        """
    ...
    def calc_objective(self, net) -> np.ndarray:
        """
        Default: Compute reward/costs from poly costs.
        Works only if defined as pandapower OPF problem and only for poly costs!
        If that is not the case, this method needs to be overwritten!
        """
        return -min_pp_costs(net)

    def calc_violations(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        valids_violations_penalties = [voltage_violation(self.net, **self.volt_pen),
    ...
    def calc_reward(self, test: bool = False) -> np.ndarray:
        """
        ...
        if self.reward_function == 'summation':
            ...
            penalties: float = -abs(sum(objectives)) * (~valids + percentage_violations)
            self.info['penalties'] = penalties

        else:
            raise NotImplementedError('This reward definition does not exist!')

        full_obj: np.ndarray = np.append(objectives, penalties)
    ...
    def _get_obs(self, obs_keys: list, add_time_obs: bool) -> np.ndarray:

        obs: list = [self.net[unit_type][column].loc[idxs].to_numpy()
                     if (unit_type != 'load' or not self.bus_wise_obs) else
                     get_bus_aggregated_obs(self.net, 'load', column, idxs) for unit_type, column, idxs in obs_keys]
    ...
    def render(self, mode='human') -> None:
        logging.warning("Rendering not implemented!")

    def get_current_actions(self) -> np.ndarray:
    ...
    def baseline_reward(self) -> float:
        """
        Compute some baseline to compare training performance with.
        In this case, use the optimal possible reward, which can be computed with the optimal power flow.
        """
...
def get_obs_space(net: pandapowerNet, obs_keys: list, add_time_obs: bool, seed: int | None, last_n_obs: int = 1, bus_wise_obs: bool = False):
    """
    ...
    if add_time_obs:
        # Time is always given as observation of length 6 in range [-1, 1] at the beginning of the observation!
    ...
    return gymnasium.spaces.Box(np.concatenate(lows, axis=0), np.concatenate(highs, axis=0), seed=seed)
...
def get_action_space(act_keys: list, seed: int):
    ...
    return gymnasium.spaces.Box(low, high, seed=seed)


def define_test_steps(test_share: float = 0.2) -> np.ndarray:
    """
...
def get_simbench_time_observation(profiles: dict, current_step: int) -> np.ndarray:
...
def get_bus_aggregated_obs(net: pandapowerNet, unit_type: str, column: str, idxs: int) -> np.ndarray:
    """
    Aggregate power values that are connected to the same bus to reduce state space.
    """
    df = net[unit_type].iloc[idxs]
    return df.groupby(['bus'])[column].sum().to_numpy()


def get_automatic_reward_scaling(net: pandapowerNet) -> float:
    cost_df = net.poly_cost

    active_power_mask = np.zeros(len(cost_df), dtype=np.bool)
    reactive_power_mask = np.zeros(len(cost_df), dtype=np.bool)
